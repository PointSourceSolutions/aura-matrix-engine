...
At each time step $t$, every node $i$ evaluates its probability of initiating an interaction (Pi(t)) based on its baseline interaction rate ($\rho_i$), current internal energy ($E_i$), and historical momentum ($M_i$):
